...
#	gcc -o $@ $^ -ly -ll
	gcc -o $@ $^ ${LDFLAGS} -ly -ll
	gcc -mmacosx-version-min=13.1 -o $@ $^ ${LDFLAGS} -ly -ll
...
LADIRfinal="Hargas-Ali"
zip_final:
	rm -rf ${LADIRfinal}
	mkdir ${LADIRfinal}
	cp regexp.l regexp.y Makefile automate.py test.1 ${LADIRfinal}
	rm -f ${LADIRfinal}.zip
	zip -r ${LADIRfinal}.zip ${LADIRfinal}
	rm -rf ${LADIRfinal}